...
#load dataset
df = pd.read_csv('./csv/crimeTime2.csv.gz', compression='gzip')
...
df = pd.concat([df[['ordinalDOW', 'Time_Minutes', 'CrimeWeight']], district_df, category_df], axis=1)
...
model.fit(X_train, y_train, 
          epochs=10, #set to 10 for time purposes
          batch_size=32, 
...
print(classification_report(y_test_labels, y_pred, target_names=category_labels))
